...
- **Convex** provides reactive queries, mutations, actions, and auth in one package. The deployment is per-developer; `VITE_CONVEX_URL` lives in `.env.local` (gitignored).
- `convex/_generated/` is regenerated by `pnpm convex:dev` and gitignored.
- One-shot seed scripts live in `scripts/` and run via `pnpm seed:champions`. They fetch Showdown's public dumps (`pokedex.json`, `items.json`, `formats.js`) to derive `regulations.legalSpecies` and `regulations.legalItems`, then invoke the internal seed mutation via `pnpm exec convex run`.
