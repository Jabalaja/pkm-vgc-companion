...
- One-shot seed scripts live in `scripts/` and run via `pnpm seed:champions`. They use Showdown's public JSON dumps to derive `regulations.legalSpecies` and call internal Convex mutations via `ConvexHttpClient`.
